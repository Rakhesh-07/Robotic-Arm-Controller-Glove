// Include BLE header files
#include <BLEDevice.h>
#include <BLEUtils.h>
#include <BLEClient.h>
#include <ESP32Servo.h>

// Define UUID - make sure it matches with main
...
// May require adjusting based on flex sensor and user
...
// Function for ALL LEDS on when fully curled
...
// This function controls the LEDS for each finger based on how much it is curled
void controlLEDs(int flexValue, int startIndex) {
    ...
    switch(startIndex){
      case 0: //Thumb
        if     (flexValue < 0)                         { no_connection(LEDindex); }
        else if(flexValue <= fully_curled[startIndex]) { full_curl(LEDindex); }
        else if(flexValue <= partial_curl[startIndex]) { partial_curling(LEDindex); }
        else if(flexValue <= slight_bend[startIndex])  { slight_curl(LEDindex); }
        else { no_connection(LEDindex); }
        break;
...
// This function will automatically connect the client if the connection drops from reseting the server
void reconnectBLE() {
...
void setup() 
{
  ...
  // 90:15:06:94:1b:fa
  // REPLACE BLEADDRESS WITH THE SERVER ADDRESS. EACH ESP32 HAS A DIFFERENT ONE
  // BLE_ADDR.ino
...
void loop() 
{
  ...
  if (pClient->isConnected() && pRemoteChar && pRemoteChar->canRead())  // Check if connected to server
  {
    ...
    if (value.length() > 0 && value.indexOf(',') != -1) {
      ...
      // Scan in received string and separate the values usign the delimiter ,
      if (sscanf(receivedBuffer, "%d,%d,%d,%d,%f,%f,%f,", 
          &flexSensors[0], &flexSensors[1], &flexSensors[2], &flexSensors[3],
          &roll, &pitch, &yaw) == 7) 
      {
        Serial.print("Received Data: "); 
        Serial.println(receivedBuffer); // Print the received data for checking

        // Pass in the flex sensor readings to control LEDS
        for (int i = 0; i < 4; i++) {
          controlLEDs(flexSensors[i], i);
        }

        // Map the roll pitch and yaw to proper servo motor positions
        rollServo = map(roll, -80, 80, 0, 180);
        pitchServo = map(pitch, -80, 80, 0, 180);
        yawServo = map(yaw, -50, 50, 0, 180);

        // Print Servo values to serial monitor for debugging
    ...
    reconnectBLE(); // Reconnect if disconnected
  }
